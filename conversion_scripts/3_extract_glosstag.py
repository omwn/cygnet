...
def main():
    ...
    for file in sorted(glob.glob(os.path.join('bin/WordNet-3.0/glosstag/merged/*.xml'))):
        ...
        for synset_xml in root.findall('synset'):
            ...
            if not 'Definition' in codes:
                print(f'No definition in {synset_xml.attrib["id"]}')
                no_def = True
    ...
    save_json('bin/concepts_to_definitions.json', {k: v.to_dict() for k, v in definitions.items()})
    # save_json('bin/example_sentences_princeton.json', {k: [v.to_dict() for v in vs] for k, vs in examples.items()})
